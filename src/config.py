...
test_mode = True
if test_mode:
    experiment_name = 'ours_best_anime_ld'
    experiment_description = 'anime ld server run'
else:
...
opt = CN()
opt.gan_type = 'SNGAN'  # choices=['BigGAN', 'ProgGAN', 'StyleGAN2','SNGAN' ,'StyleGAN']
...
opt.algo = CN()
opt.algo.ours = CN()
opt.algo.ours.model_name = 'sngan_animefaces'  # choices = ['pggan_celebahq1024',stylegan_animeface512,stylegan_car512,stylegan_cat256]
opt.algo.ours.initialisation = 'latent_discovery'  # choices = ['closed_form', 'latent_discovery', 'gan_space]
opt.algo.ours.num_steps =200001
opt.algo.ours.batch_size = 128
opt.algo.ours.num_steps =20001
opt.algo.ours.batch_size = 16
>>>>>>> f69f2461210529a70076324acff8e5e11f84ddb1
...
opt.algo.ours.num_directions = 128
opt.algo.ours.latent_dim = 128
...
def save_config(config, opt):
    ...
    cwd = os.path.dirname(os.getcwd()) + 'results/anime/latent_discovery_ours'  # project root
    opt.result_dir = cwd
