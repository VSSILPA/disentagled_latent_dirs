...
from infogan_components import Linear

import pylab as plt


class InfoGAN:
    def __init__(self, gen, dis, embedding_len, z_len, c1_len, c2_len, c3_len, device):
        ...
        self.Q_cat = Linear(embedding_len, c1_len).to(self.device)
        self.qcat_optim = optim.Adam(self.Q_cat.parameters(), lr=2e-4)
        if c2_len:
            self.Q_con = Linear(embedding_len, c2_len).to(self.device)
            self.qcon_optim = optim.Adam(self.Q_con.parameters(), lr=2e-4)
        if c3_len:
            self.Q_bin = Linear(embedding_len, c3_len).to(self.device)
    ...
    def train_all(self, train_loader):
        ...
        for epoch in range(100):
            ...
            for i, (data, targets) in enumerate(train_loader):
                ones = Variable(torch.ones(data.size()[0], 1)).to(self.device)
                zeros = Variable(torch.zeros(data.size()[0], 1)).to(self.device)

                z_dict = self.get_z(data.size()[0])
                z = torch.cat([z_dict[k] for k in z_dict.keys()], dim=1)

                data = Variable(data.float().to(self.device)) / 255
                ...
                loss_dis = mse(out_dis, ones) - torch.sum(targets * c1) / (
                            torch.sum(targets) + 1e-3)  # Loss for real MNIST
                ...
                loss_dis.backward(
                    retain_graph=True)  # We need PyTorch to retain the graph buffers so we can run backward again later
                ...
                pb.set_postfix(loss_dis=loss_dis.cpu().data.numpy()[0], loss_gen=loss_gen.cpu().data.numpy()[0],
                               loss_q=loss_q.cpu().data.numpy()[0])
    ...
    def get_z(self, length, sequential=False):
        ...
        z = {}
        if self.z_len:
            z['z'] = Variable(torch.randn(length, self.z_len)).to(self.device)

        if self.c1_len:
            if sequential:
                cat_noise = Variable(torch.arange(0, self.c1_len).repeat(length // self.c1_len).long()).to(self.device)
            else:
                cat_noise = Variable(torch.multinomial(weights, num_samples=length, replacement=True)).to(
                    self.device).view(-1)
            onehot_noise = Variable(torch.zeros(length, self.c1_len)).to(self.device)
            onehot_noise.data.scatter_(1, cat_noise.data.view(-1, 1), 1)
            z['cat'] = onehot_noise

        if self.c2_len:
            # z['con'] = Variable(torch.randn(length, c2_len)).cuda()
            z['con'] = Variable(torch.rand(length, self.c2_len)).to(self.device) * 2 - 1

        if self.c3_len:
            z['bin'] = Variable(torch.bernoulli(0.5 * torch.ones(length, self.c3_len))).to(self.device).float()
    ...
    def load(self, directory):
        self.gen.load_state_dict(torch.load(directory + 'gen.torch', map_location=self.device))
        self.dis.load_state_dict(torch.load(directory + 'dis.torch', map_location=self.device))
        if self.c1_len:
            self.Q_cat.load_state_dict(torch.load(directory + 'qcat.torch', map_location=self.device))
        if self.c2_len:
            self.Q_con.load_state_dict(torch.load(directory + 'qcon.torch', map_location=self.device))
        if self.c3_len:
            self.Q_bin.load_state_dict(torch.load(directory + 'qbin.torch', map_location=self.device))