...
def load_deformator(opt, G):
    model_name = opt.algo.ours.model_name
    directions = torch.load(os.path.join(DEFORMATOR_CHECKPOINT_DIR, model_name, 'deformator_0.pt'), map_location=torch.device('cpu'))['log_mat_half']
    ...
    deformator.log_mat_half.data = torch.FloatTensor(directions)
    deformator.cuda()
    return deformator